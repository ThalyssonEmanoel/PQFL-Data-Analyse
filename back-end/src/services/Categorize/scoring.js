...
function stripQuestionIdSuffix(fieldName) {
...
function normalizeAnswerToScore(value) {
...
function buildNormalizedFieldIndex(flatPayload) {
  const index = new Map();
  for (const [key, value] of Object.entries(flatPayload)) {
    const normalizedKey = normalizeFieldName(key);
    const entry = { originalKey: key, value };
...
function findFieldByCandidate(flatPayload, normalizedIndex, candidate) {
...
function findFieldByCandidates(flatPayload, normalizedIndex, candidates) {
...
function buildFactorDiagnostics(flatPayload, normalizedIndex) {
  const diagnostics = [];
...
function mapPayloadKeyToCategory(normalizedFieldName, categoryFieldHints) {
...
function isMatchingHint(fieldName, hints) {
...
function scoreIdentityField(fieldName, value, hints, identityType) {
...
function getBestIdentityValueByHint(flatPayload, hints, identityType) {
  let bestValue = null;
...
function buildFallbackProducerId(rawPayload) {
...
function getPrimaryProducerNameValue(flatPayload, normalizedIndex) {
...
function getHighestNumericValueByHint(normalizedIndex, hints) {
...
function hasResiduePresence(normalizedIndex, residueHints) {
...
function buildCategoryHintMap(customHints) {
  const hintMap = {};
...
function classifyProducer(totalScore) {
  if (totalScore >= 80) {
    return "G1";
  }
...
export function mapAndScoreProducers(rawPayloads, customOptions = {}) {
